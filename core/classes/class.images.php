...
class images
{
        ...
        static private final function any2any( $file, $config = array() )
        {
                ...
                if( self::IMAGES_CONVERT_EXT == 'png' ){  imagepng( $im, $_newfile, self::PNG_COMPRESS_LEVEL ); }
                if( self::IMAGES_CONVERT_EXT == 'jpeg' ){ imagejpeg( $im, $_newfile, self::JPEG_QUALITY_LEVEL ); }
        ...
        static private final function watermark( $file )
        {
                ...
                if( self::IMAGES_CONVERT_EXT == 'png' ){  $im = imagecreatefrompng( $file ); }
                if( self::IMAGES_CONVERT_EXT == 'jpeg' ){ $im = imagecreatefromjpeg( $file ); }
                ...
                if( self::IMAGES_CONVERT_EXT == 'png' ){  imagepng( $im, $file, self::PNG_COMPRESS_LEVEL ); }
                if( self::IMAGES_CONVERT_EXT == 'jpeg' ){ imagejpeg( $im, $file, self::JPEG_QUALITY_LEVEL ); }
                return true;
        }
        ...
        static private final function makemini( $file, $x = 200, $y = 100, $prop = false )
        {
                ...
                if( self::IMAGES_CONVERT_EXT == 'png' ){  imagepng( $new_im, $file, self::PNG_COMPRESS_LEVEL ); }
                if( self::IMAGES_CONVERT_EXT == 'jpeg' ){ imagejpeg( $new_im, $file, self::JPEG_QUALITY_LEVEL ); }
                return $file;
        }
